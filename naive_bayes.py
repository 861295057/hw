...
def Train(trainset,train_labels):
    ...
    #条件概率具有：类别，特征，状态取值这一个特征，因此应该使用三维列表来存储。np初始化结构。
    ...
    for i in range(len(train_labels)):
        ...
        #防止出现为零的情况，因为只是大小比较，只要相对大小不改变即可，因此不必写为标准概率。
    ...
    for i in range(class_num):
        for j in range(feature_len):
            ...
            # 计算0，1像素点对应的条件概率
            #使用贝叶斯估计，lamda = 1的情况，*10000是为了之后转化为整形，防止数据溢出
            probalility_0 = (pix_0+1)/(pix_0+pix_1+2) * 10000
            probalility_1 = (pix_1+1)/(pix_0+pix_1+2) * 10000
...
def calculate_probability(img,label):
    #防止出现为零的情况，因为只是大小比较，只要相对大小不改变即可！
